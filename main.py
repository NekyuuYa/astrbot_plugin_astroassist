...
@register("astrbot_plugin_astroassist", "NekyuuYa", "晴天钟助手 - 专业天文气象看板", "0.8.10")
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                try:
                    ...
                    tr = await client.get(t_url, timeout=10.0)
                    if tr.status_code == 200: t_data = tr.json()
                except: logger.warning("7Timer! Down")
                ...
                init_dt = None
                if t_data.get("init"): init_dt = datetime.datetime.strptime(t_data["init"], "%Y%m%d%H").replace(tzinfo=datetime.timezone.utc)
                ...
                for i in range(len(hourly["time"])):
                    ...
                    if init_dt:
                        ...
                        if 0 <= timer_idx < len(timer_series):
                            astro = timer_series[timer_idx]
                            seeing_val, trans_val = astro.get("seeing", 5), astro.get("transparency", 5)
                            seeing_color, trans_color = get_m3_color(seeing_val, "astro")[0], get_m3_color(trans_val, "astro")[0]
                            astro_cls = get_m3_color(seeing_val, "astro")[1]
                    ...
                    all_rows.append({
                        ...
                        "seeing_val": seeing_val, "seeing_color": seeing_color, "seeing_cls": astro_cls,
                        "trans_val": trans_val, "trans_color": trans_color, "trans_cls": astro_cls,
                ...
                template_str, save_path = self._load_template(), os.path.abspath(f"data/plugin_data/astrbot_plugin_astroassist/forecast_v810.png")
